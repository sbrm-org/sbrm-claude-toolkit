"""Handshake job search via Playwright headed browser.

Employer portal login required. Disabled by default (mostly student/entry-level).

Usage:
    echo '<snapshot>' | python3 search_handshake.py --store \
...
def build_search_url(keyword: str) -> str:
    """Build Handshake employer search URL."""
    return (
        f"https://app.joinhandshake.com/emp/jobs"
...
def parse_snapshot(snapshot_text: str) -> list[dict]:
    """Parse a Playwright snapshot of Handshake search results."""
    results = []
    current = {}
    ...
    for line in snapshot_text.strip().split("\n"):
        ...
        title_match = re.search(
            r'\[([^\]]+)\]\((https://app\.joinhandshake\.com/[^\)]*(?:job|posting)[^\)]*)\)',
            line
        )
        if title_match:
        ...
        if "employer" not in current:
            if (not line.startswith(("http", "[", "!", "#", "$"))
                    ...
                    and len(line) > 2):
                current["employer"] = re.sub(r'[*_`]', '', line).strip()
                continue
        ...
        if "employment_type" not in current:
            ...
            elif re.search(r'\binternship\b', line, re.I):
...
def store_results(db_path: str, results: list[dict], role_id: int, run_id: int) -> dict:
    """Store parsed Handshake results in SQLite."""
